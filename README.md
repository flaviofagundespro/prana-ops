...
**Agent console — operations layer for long-running AI coding agents, local or remote.**
...
You give an autonomous coding agent a task, it runs for an hour in a `tmux` session on your own
machine or a remote host, and at some point it stops and asks a question.
...
- **Terminal grid** (1/2/4/6 panes, xterm.js) over `tmux` sessions on SSH hosts or on the
  operator's own machine. SSH uses one multiplexed connection per host — growing into a bounded
  pool only when the server refuses a channel (ADR 003); local terminals use a real PTY.
...
flowchart LR
  subgraph local["Operator machine"]
    ...
    LT["local tmux sessions<br/>ckpt-*"]
    LA["local Claude Code / Codex"]
    UI <-->|"WebSocket<br/>typed protocol"| SRV
    SRV -->|"PTY + child process"| LT
    LA --- LT
  end
...
An environment is either **SSH** or **Local**. In Settings, choose Local and give it a name
(for example, `Ryzen`). No host, user, key, password, or local SSH daemon is needed.

Local terminal panes run the same `ckpt-*` tmux flow through a pseudoterminal. That matters for
full-screen programs such as Claude Code and Codex: they receive a real TTY, input, output and
resize events. Short control queries use ordinary child-process pipes so their output stays clean.
The existing SSH connection pool is not involved.

Prana OPS launches the operator's existing shell. It does **not** install, authenticate,
reconfigure, copy, or read the configuration of Claude Code, Codex, aliases, SSH keys or logins.
Closing a browser pane only detaches its PTY client; it does not kill the corresponding tmux
session. Delete the session explicitly when it is meant to end.
...
| [006](docs/adr/006-local-pty-is-a-transport.md) | Local is a first-class PTY transport, not `localhost` over SSH | Local programs already have a shell; an SSH daemon and fake credentials add complexity without value |
...
```
505 tests · 3 workspaces · ~4s
```

| Workspace | Tests | Approach |
|---|---|---|
| `server` | 227 | SSH and local transports are mocked end to end — **no unit test opens a real SSH connection or starts an agent**. Saturation, refusal, PTY lifecycle and routing are reproduced with fakes. |
| `web` | 156 | Component tests over the typed WebSocket protocol, including the Local/SSH Settings flow |
| `watcher` | 122 | Includes a replay of the exact production timeline that exposed the precedence bug |
...
**Requirements:** Linux, Node 18+, and `tmux`. SSH environments additionally need an SSH key on
disk and a remote host with `tmux`. `better-sqlite3`, `ssh2`, and `node-pty` compile native
...
- **Local agent configuration** is not managed by the cockpit. Local shells use the already
  installed programs and configuration of the operator.
...
with a single runtime dependency. Node · Express · `ws` · `ssh2` · `node-pty` · `better-sqlite3`
on the server; React · Vite · xterm.js on the front end.
